...
public class AlexandriaMarkupClient implements AutoCloseable {
  ...
  public RestResult<String> getLMNL(UUID documentUUID) {
    WebTarget path = documentTarget(documentUUID).path(DocumentsResource.SubPaths.LMNL);
    return stringResult(path);
  }

  public RestResult<String> getDocumentLaTeX(UUID documentUUID) {
    WebTarget path = documentTarget(documentUUID).path(DocumentsResource.SubPaths.LATEX);
    return stringResult(path);
  }

  public RestResult<String> getMarkupDepthLaTex(UUID documentUUID) {
    WebTarget path = documentTarget(documentUUID).path(DocumentsResource.SubPaths.MARKUPDEPTH);
    return stringResult(path);
  }

  public RestResult<String> getMatrixLaTex(UUID documentUUID) {
    WebTarget path = documentTarget(documentUUID).path(DocumentsResource.SubPaths.MATRIX);
    return stringResult(path);
  }

  public RestResult<String> getKdTreeLaTex(UUID documentUUID) {
    WebTarget path = documentTarget(documentUUID).path(DocumentsResource.SubPaths.KDTREE);
    return stringResult(path);
  }
